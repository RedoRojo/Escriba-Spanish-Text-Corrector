...
const manuscript = document.getElementById('manuscript') as HTMLDivElement;
...
const finalSection = document.getElementById('final-section') as HTMLDivElement;
const finalText = document.getElementById('final-text') as HTMLDivElement;
const copyBtn = document.getElementById('copy-btn') as HTMLButtonElement;
const emptyState = document.getElementById('empty-state') as HTMLDivElement;

const popover = document.getElementById('error-popover') as HTMLDivElement;
const popoverType = document.getElementById('popover-type') as HTMLSpanElement;
const popoverErrNum = document.getElementById('popover-err-num') as HTMLSpanElement;
const popoverOriginal = document.getElementById('popover-original') as HTMLParagraphElement;
const popoverCorrection = document.getElementById('popover-correction') as HTMLParagraphElement;
const popoverExplanation = document.getElementById('popover-explanation') as HTMLParagraphElement;
const popoverAccept = document.getElementById('popover-accept') as HTMLButtonElement;
const popoverIgnore = document.getElementById('popover-ignore') as HTMLButtonElement;
...
interface ErrorTypeRegistry {
  [key: string]: ErrorTypeInfo;
}

/* ── Error type registry (populated from API on load) ── */
let errorTypeRegistry: ErrorTypeRegistry = {};
/* ── State ── */
let currentText: string = '';
let errors: ErrorItem[] = [];
let errorStates: ErrorState[] = [];       // { status: 'pending'|'accepted'|'ignored', error }
let errorPositions: ErrorPosition[] = [];    // { start, end, index }
let activeErrorIndex: number = -1;
...
async function initErrorTypes(): Promise<void> {
  try {
...
function locateErrors(text: string, errs: ErrorItem[]): ErrorPosition[] {
...
function renderManuscript(): void {
  ...
  textContent.querySelectorAll('.error-highlight[data-error-idx]').forEach((el) => {
    el.addEventListener('click', (e) => {
      ...
      const idx = parseInt(target.dataset.errorIdx || '0');
      openPopover(idx, target);
    });
  });
}
...
/* ── Popover ── */
function openPopover(idx: number, anchorEl: HTMLElement): void {
...
function handleDocClick(e: MouseEvent): void {
  ...
  if (popover.classList.contains('show') && !popover.contains(target) && !target?.closest('.error-highlight')) {
    closePopover();
  }
}

/* ── Resolve error (accept / ignore) ── */
function resolveError(status: 'accepted' | 'ignored'): void {
